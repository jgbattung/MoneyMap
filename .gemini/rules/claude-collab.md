...
4. **Final Cleanup:** Once you confirm the verification is complete and successful, you must automatically move the `-spec.md`, `-plan.xml`, and `-verification.md` files to `docs/archive/`, then delete any `cpython-311.pyc` files. Then, create a final atomic commit for this cleanup (e.g., `docs(feature): archive docs`) and `git push` it to the remote branch.
